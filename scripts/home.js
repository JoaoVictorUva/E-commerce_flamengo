...
axios.get('../scripts/api/apiFla.json')
    .then(response => {
        // Produtos masculinos
        const masculinos = response.data.filter(item => item.tipo === 'Masculino').slice(0, 4);
        displayProducts(masculinos, 'product-masculinos');

        // Produtos femininos
        const femininos = response.data.filter(item => item.tipo === 'Feminino').slice(0, 4);
        displayProducts(femininos, 'product-femininos');

        // Produtos infantis
        const infantis = response.data.filter(item => item.tipo === 'Infantil').slice(0, 4);
        displayProducts(infantis, 'product-infantil');
    })
    .catch(error => {
